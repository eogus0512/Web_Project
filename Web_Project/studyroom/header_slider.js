...
var slides = new Array('./img/slider/slider1.png',
...
function transition(id)
{
	...
	var newLayer = $('<div class="mosaic-slide">').html(generateGrid({rows:6,cols:13,image:slides[id-1]}));
	...
	for(var i=0;i<param.rows;i++)
	{
		for(var j=0;j<param.cols;j++)
		{
			tmp = $('<div>', {
					"class":"tile",
					"css":{
						"background":'white url('+param.image+') no-repeat '+(-j*tabwidth)+'px '+(-i*tabheight)+'px'
					}
			});
